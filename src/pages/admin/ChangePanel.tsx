...
export const ChangePanel = ({ responses }: { responses: readonly ConsultationResponse[] }) => {
  ...
  const comparison = useMemo(
    () => compareRounds(questionnaire, responses.filter((response) => !response.isTestData), rounds),
    ...
    <div className="mt-6 grid gap-5">
      <section className={accentPanel}>
        ...
        <p className="mt-2 text-meta text-ink-soft">
          Responses are anonymous, so each round is a separate picture of the industry — this compares pictures, not
          the same people over time. Check <strong>who answered</strong> below before reading a change as a change of
          mind: a different mix of respondents moves the numbers too. Figures from fewer than {THIN} people are greyed.
        </p>
      </section>
                ...
                <tbody>
                  {block.rows.map((row) => (
                    <tr key={row.id} className="border-b border-line last:border-0">
                      ...
                      {comparison.hasBaseline && comparison.hasReview && (
                        <td className="py-2 pl-3 text-right font-semibold">{formatChange(row.change, block.measure)}</td>
                      )}
                    </tr>
                  ))}
                </tbody>
